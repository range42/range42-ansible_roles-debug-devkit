...
show_example() {
  ...
  local STDIN_JSON_DATA=(
    ...
    '{"proxmox_node":"px-testing", "vm_id":112, "vm_snapshot_description":"MY_DESCRIPTION" } '
    '{"proxmox_node":"px-testing", "vm_id":112, "vm_snapshot_name":"MY_VM_SNAPSHOT", "vm_snapshot_description":"MY_DESCRIPTION" } '
...
if [[ "${RANGE42_PROXMOX_API_FORCE:-auto}" != "off" ]]; then
  if proxmox__inc.api_reachable.sh ; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "proxmox API reachable - delegating to proxmox_snapshot_vm.vm_id.delete_snapshot_with_api.to.jsons.sh"
    exec proxmox_snapshot_vm.vm_id.delete_snapshot_with_api.to.jsons.sh "$@"
  else
