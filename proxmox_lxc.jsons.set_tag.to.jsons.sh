...
show_example() {
  ...
  local STDIN_JSON_DATA=(
    '{"vm_id":100, "lxc_tag_name": "group_01"} '
    '{"vm_id":100, "lxc_tag_name": "group_01,group_02"} '
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo
  echo
  echo "  $(basename "$0") - set tag/label on LXC "
  echo
  echo OPTIONS
  echo
...
JSON_LINE_REQ=$(devkit_proxmox.STDIN.stdin_or_jsons.to.jsons.sh "INT::vm_id" "STR::lxc_tag_name" "STR::proxmox_node" "STR::action")
