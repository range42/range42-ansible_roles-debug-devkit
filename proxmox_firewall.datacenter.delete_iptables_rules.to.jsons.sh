...
#
# Delete ONE firewall rule of the DATACENTER, by position.
#
...
show_example() {
  ...
  local STDIN_JSON_DATA=(
    \
    '{"proxmox_node":"px-testing","dc_fw_pos":0}'
  )
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Delete specified iptables rule - datacenter firewall - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
