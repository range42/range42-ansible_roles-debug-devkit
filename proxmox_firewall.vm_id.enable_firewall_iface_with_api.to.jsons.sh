...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####
# proxmox_firewall.vm_id.enable_firewall_iface_with_api.to.jsons.sh
# Direct Proxmox HTTPS API variant of proxmox_firewall.vm_id.enable_firewall_iface.to.jsons.sh
#
# The same steps as the role action firewall_vm_iface_enable, field for field, on ONE
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Enable the firewall flag of ONE network card of a vm - direct Proxmox HTTPS API call ($ACTION)"
  echo
  echo OPTIONS
  echo
