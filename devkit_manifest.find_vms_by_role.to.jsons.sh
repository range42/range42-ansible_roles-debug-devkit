...
show_example() {
  ...
  echo "  $(basename "$0") \"\$0\" \"admin\" | proxmox_snapshot_vm.vm_id.create_snapshot.to.jsons.sh"
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ] || [ $# -lt 2 ]; then
  ...
  echo
  echo NAME
  echo "  $(basename "$0") - list full VM entries (JSON lines) matching a role"
  echo
  echo OPTIONS
  echo
  ...
  echo "  then outputs one compact JSON object per line for every VM whose role"
  echo "  matches <role>. Output is jsons (newline-delimited JSON) compatible with"
  echo "  the rest of the devkit pipeline."
...
jq -c --arg r "$role" '.vms[] | select(.role == $r)' "$manifest"
