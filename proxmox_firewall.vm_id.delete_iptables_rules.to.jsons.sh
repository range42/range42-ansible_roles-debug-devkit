...
show_example() {
  ...
  local STDIN_JSON_DATA=(
    # INPUT
    '{"vm_id":100, "vm_fw_pos": 1}'
    '{"vm_id":100, "vm_fw_pos": 1,  "proxmox_node":"px-testing"}'
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Delete specificied iptables rule - vm firewall - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
