...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####
#
# delete an SDN zone, once it has no vnet left
#
# LAST of the three deletions : subnet -> vnet -> zone -> apply.
# Fails as long as the zone still carries a vnet.
#
# A zone is SHARED between scenarios. Removing it is right when tearing down a whole test
# cluster, and wrong when retiring a single scenario, which only needs its vnet gone.
#
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####

set -euo pipefail
ACTION="network_delete_sdn_zone"
...
if [ "${1:-}" = "-h" ] || [ "${1:-}" = "--help" ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - delete an SDN zone, once it has no vnet left - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
