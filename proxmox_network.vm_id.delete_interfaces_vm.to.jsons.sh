...
show_example() {
  ...
  local STDIN_JSON_DATA=(
    ...
    '{ "vm_id":100, "vm_vmnet_id":0, "proxmox_node":"px-testing"}'
...
JSON_LINE_REQ=$(devkit_proxmox.STDIN.stdin_or_jsons.to.jsons.sh "INT::vm_vmnet_id" "INT::vm_id" "STR::proxmox_node" "STR::action")
...
printf '%s\n' "$JSON_LINE_REQ" | while IFS=$'\n' read -r CURRENT_JSON_LINE; do
  ...
  NEW_CURRENT_JSON_LINE=$(printf '%s\n' "$CURRENT_JSON_LINE" |
    jq -c --arg jq_vm_vmnet_id "$IS_VM_VMNET" '. + {  ("vm_vmnet_id"): $jq_vm_vmnet_id } ')
