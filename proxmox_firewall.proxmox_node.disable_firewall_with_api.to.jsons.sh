...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####
# proxmox_firewall.proxmox_node.disable_firewall_with_api.to.jsons.sh
# Direct Proxmox HTTPS API variant of proxmox_firewall.proxmox_node.disable_firewall.to.jsons.sh
#
# The same steps as the role action firewall_node_disable, field for field :
#   1. GET ${API_URL}/nodes/${NODE}/firewall/rules     the chain, to SAY how many active rules are
...
#   2. PUT ${API_URL}/nodes/${NODE}/firewall/options   enable 0
#
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Turn the node firewall off, saying what becomes inert - direct Proxmox HTTPS API call ($ACTION)"
  echo
  echo OPTIONS
  echo
...
while IFS= read -r LINE ; do
  ...
  _trace "$(printf '%s' "$VERDICT" | jq -r '"TURNING OFF the node firewall : " + (.total | tostring) + " rule(s) in the chain, " + (.active | tostring) + " of them ACTIVE and about to stop applying. This is not a refusal, it is the count, so that turning off a whole policy does not look like turning off nothing."')"

  # 2. the write
  _api_put "${API_URL}/nodes/${NODE}/firewall/options" '{"enable":0}'
  if [[ "$HTTP_CODE" != "200" ]]; then
    _error "PUT node firewall/options enable=0 failed (http ${HTTP_CODE}) : ${BODY}"
    exit 1
  fi
