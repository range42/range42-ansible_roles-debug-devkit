...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####
# PR-NN
# proxmox_snapshot_vm.vm_id.revert_snapshot_with_api.to.jsons.sh
# Direct Proxmox HTTPS API variant of proxmox_snapshot_vm.vm_id.revert_snapshot.to.jsons.sh
#
...
show_example() {
  ...
  echo
  echo "    echo '{\"vm_id\":100, \"vm_snapshot_name\":\"before-deploy\"}' | $(basename "$0")"
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - revert a VM to a snapshot - direct Proxmox HTTPS API call ($ACTION)"
  echo
  echo OPTIONS
  echo
...
while IFS= read -r LINE ; do
  ...
  POST_RESP="$(curl -sk -X POST -H "$AUTH_HEADER" "${BASE_URL}/qemu/${VM_ID}/snapshot/${SNAP_NAME_ENC}/rollback" || true)"
  UPID="$(printf '%s' "$POST_RESP" | jq -r '.data // empty')"
  if [[ -z "$UPID" || "$UPID" == "null" ]]; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "POST rollback did not return a UPID for vm_id ${VM_ID} snap '${SNAP_NAME}'. response: $POST_RESP"
    continue
  fi
  ...
  if [[ "${TASK_STATUS:-}" != "stopped" ]]; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "rollback task ${UPID} did not complete within ${POLL_TIMEOUT_SECONDS}s ; emitting result anyway."
  fi
