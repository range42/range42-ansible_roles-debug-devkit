...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Start vm_id vm - direct Proxmox HTTPS API call ($ACTION)"
  echo
  echo OPTIONS
  echo
...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####
# parse flags : only --json or --text ; vm_id comes from stdin (pipe-only)
...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####
# sanity checks on workspace env vars
...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####
# decrypt vault ONCE, extract Proxmox API fields
...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####
# stdin loop : extract vm_id from each line (plain text OR JSON line with .vm_id)
...
while IFS= read -r LINE ; do
  ...
  POST_RESP="$(curl -sk -X POST -H "$AUTH_HEADER" "${BASE_URL}/qemu/${VM_ID}/status/start" || true)"
  UPID="$(printf '%s' "$POST_RESP" | jq -r '.data // empty')"
  if [[ -z "$UPID" || "$UPID" == "null" ]]; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "POST start did not return a UPID for vm_id ${VM_ID}. response: $POST_RESP"
    continue
  fi
  ...
  if [[ "${TASK_STATUS:-}" == "running" ]]; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "task ${UPID} still running after ${POLL_TIMEOUT_SECONDS}s ; fetching current status anyway."
  fi
