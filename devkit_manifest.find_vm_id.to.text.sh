...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ] || [ $# -lt 2 ]; then
  ...
  echo
  echo NAME
  echo "  $(basename "$0") - lookup vm_id of a VM in the closest scenario manifest"
  echo
  echo OPTIONS
  echo
  ...
  echo "  then outputs the vm_id of the VM whose vm_name matches <vm_name>."
...
vm_id=$(jq -r --arg n "$vm_name" '.vms[] | select(.vm_name == $n) | .vm_id' "$manifest")
