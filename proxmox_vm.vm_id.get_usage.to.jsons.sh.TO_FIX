...
show_example() {
  ...
  printf '%s | %s\n' "$(devkit_utils.text.echo_json_helper.to.text.sh "${STDIN_JSON_DATA[-1]}")" "$(basename "$0") vm_name --json"
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - get VM usage  - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
