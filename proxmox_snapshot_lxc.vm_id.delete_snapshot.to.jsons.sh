...
show_example() {
  ...
  local STDIN_JSON_DATA=(
    ...
    '{"proxmox_node":"px-testing", "vm_id":100, "lxc_snapshot_name":"MY_LXC_SNAPSHOT"}'
  )
  ...
  printf '%s | %s\n' "$(devkit_utils.text.echo_json_helper.to.text.sh "${STDIN_JSON_DATA[-1]}")" "$(basename "$0") "

  echo ""
  echo "    cat /tmp/lxc_list.json | $(basename "$0")"
  echo ""
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo
  echo
  echo "  $(basename "$0") - delete LXC snapshot - require vm_id  - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
...
JSON_LINE_REQ=$(devkit_proxmox.STDIN.stdin_or_jsons.to.jsons.sh "INT::vm_id" "STR::lxc_snapshot_name" "STR::lxc_snapshot_description" "STR::lxc_name" "STR::proxmox_node" "STR::action")
...
printf '%s\n' "$JSON_LINE_REQ" | while IFS=$'\n' read -r CURRENT_JSON_LINE; do
  ...
  if [[ -z "$IS_LXC_SNAPSHOT_NAME" ]]; then # no associated vm_id ?
    ...
    C_LXC_LAST_SNAPSHOT_NAME=$(printf '%s\n' "$CURRENT_JSON_LINE" |
      proxmox_snapshot_lxc.vm_id.list_snapshot.to.jsons.sh |
      jq -s -r -c ".[-1].lxc_snapshot_parent")
    ...
    NEW_CURRENT_JSON_LINE=$(
      printf '%s\n' "$CURRENT_JSON_LINE" |
        jq -c \
          --arg jq_lxc_snapshot_name_v "$C_LXC_LAST_SNAPSHOT_NAME" \
          --arg jq_lxc_name_v "$C_VM_NAME" \
          '
          . + {
