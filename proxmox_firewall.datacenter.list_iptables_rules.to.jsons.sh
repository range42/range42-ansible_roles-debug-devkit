...
#
# Read the firewall rule chain of the DATACENTER.
#
# The read the role had at no level above the guest. Proxmox applies the datacenter rules
# to every node's host chain, so this chain is what covers a node the moment it joins -
# including a node nobody has configured yet.
#
# READ dc_fw_enable, NOT JUST THE PRESENCE OF A RULE
...
set -euo pipefail
ACTION="firewall_dc_list_iptables_rule"
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - List iptables rules - datacenter firewall - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
