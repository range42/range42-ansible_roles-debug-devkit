...
#
# Turn the DATACENTER firewall OFF.
#
# It does not refuse - turning a firewall off locks nobody out, it removes protection, and
# that is a legitimate operation. What it does is READ the chain first and say how many
# ACTIVE rules are about to stop applying, so that turning off a whole cluster-wide policy
# does not look like turning off nothing.
#
# This is also the way back from a guard that refused : the enable actions at node and
# datacenter level refuse to arm without a management path, and this is how you undo an
# arming that should not have happened.
#
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Disable datacenter firewall - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
...
if [[ "${RANGE42_PROXMOX_API_FORCE:-auto}" != "off" ]]; then
  if proxmox__inc.api_reachable.sh ; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "proxmox API reachable - delegating to proxmox_firewall.datacenter.disable_firewall_with_api.to.jsons.sh"
    exec proxmox_firewall.datacenter.disable_firewall_with_api.to.jsons.sh "$@"
  else
