...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo
  echo NAME
  echo "  $(basename "$0") - delete LXC - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
...
printf '%s\n' "$JSON_LINE_REQ" | while IFS=$'\n' read -r CURRENT_JSON_LINE; do
  ...
  LXC_NAME=$(
    printf '%s\n' "$CURRENT_JSON_LINE" |
      proxmox_lxc.vm_id.list_lxc_and_extract_vm_name.to.jsons.sh |
      jq -r '.lxc_name // empty'
  )
  ...
  NEW_CURRENT_JSON_LINE=$(
    printf '%s\n' "$CURRENT_JSON_LINE" |
      jq -c --arg jq_lxc_name_v "$LXC_NAME" '. + { ("lxc_name"): $jq_lxc_name_v }'
  )
