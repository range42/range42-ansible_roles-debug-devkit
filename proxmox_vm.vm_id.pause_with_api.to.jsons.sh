...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Pause (suspend) vm_id vm - direct Proxmox HTTPS API call ($ACTION)"
  echo
  echo OPTIONS
  echo
...
while IFS= read -r LINE ; do
  ...
  POST_RESP="$(curl -sk -X POST -H "$AUTH_HEADER" "${BASE_URL}/qemu/${VM_ID}/status/suspend" || true)"
  UPID="$(printf '%s' "$POST_RESP" | jq -r '.data // empty')"
  if [[ -z "$UPID" || "$UPID" == "null" ]]; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "POST suspend did not return a UPID for vm_id ${VM_ID}. response: $POST_RESP"
    continue
  fi
  ...
  if [[ "${TASK_STATUS:-}" != "stopped" ]]; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "suspend task ${UPID} did not complete within ${POLL_TIMEOUT_SECONDS}s ; reading status anyway."
  fi
