...
FIRST_READ=$(printf '%s\n' "$INPUT_JSON" | proxmox_firewall.proxmox_node.list_iptables_rules.to.jsons.sh)
...
while : ; do
  ...
  if [ "$ROUND" -gt "$BOUND" ]; then
    REMAINING=$(printf '%s\n' "$INPUT_JSON" | proxmox_firewall.proxmox_node.list_iptables_rules.to.jsons.sh | grep -c . || true)
  ...
  TARGET=$(printf '%s\n' "$INPUT_JSON" | proxmox_firewall.proxmox_node.list_iptables_rules.to.jsons.sh | jq -s -c 'if length == 0 then empty else .[0] end')
