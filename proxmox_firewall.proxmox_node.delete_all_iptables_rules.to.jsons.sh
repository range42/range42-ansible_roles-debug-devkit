...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Delete ALL iptables rules - node firewall - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
...
FIRST_READ=$(printf '%s\n' "$INPUT_JSON" | proxmox_firewall.proxmox_node.list_iptables_rules.to.jsons.sh | _lines)
...
while : ; do
  ...
  if [ "$ROUND" -gt "$BOUND" ]; then
    REMAINING=$(printf '%s\n' "$INPUT_JSON" | proxmox_firewall.proxmox_node.list_iptables_rules.to.jsons.sh | _lines | grep -c . || true)
  ...
  TARGET=$(printf '%s\n' "$INPUT_JSON" | proxmox_firewall.proxmox_node.list_iptables_rules.to.jsons.sh | _lines | jq -s -c 'if length == 0 then empty else .[0] end')
  ...
  if [[ "$OUTPUT_JSON" == true ]]; then
    ...
    printf '%s\n' "$TARGET" |
      jq -c '{ proxmox_node, node_fw_pos }' |
      proxmox_firewall.proxmox_node.delete_iptables_rules.to.jsons.sh --json

  else

    printf '%s\n' "$TARGET" |
      jq -c '{ proxmox_node, node_fw_pos }' |
      proxmox_firewall.proxmox_node.delete_iptables_rules.to.jsons.sh --text
