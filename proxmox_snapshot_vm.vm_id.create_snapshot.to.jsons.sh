...
if [[ "${RANGE42_PROXMOX_API_FORCE:-auto}" != "off" ]]; then
  if proxmox__inc.api_reachable.sh ; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "proxmox API reachable - delegating to proxmox_snapshot_vm.vm_id.create_snapshot_with_api.to.jsons.sh"
    exec proxmox_snapshot_vm.vm_id.create_snapshot_with_api.to.jsons.sh "$@"
  else
