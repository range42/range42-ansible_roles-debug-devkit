...
set -euo pipefail
ACTION="firewall_vm_iface_enable"
...
show_example() {
  ...
  echo
  echo "     It sets firewall=1 on the card, by editing the net<N> string it read - so the MAC,"
  ...
  echo
  echo "     It also switches PVE MAC anti-spoof ON : the same flag carries both, and they cannot be separated."
  echo
  echo "  :: SEE ALSO"
  echo
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Enable the firewall flag of ONE network card of a vm - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
...
if [[ "${RANGE42_PROXMOX_API_FORCE:-auto}" != "off" ]]; then
  if proxmox__inc.api_reachable.sh ; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "proxmox API reachable - delegating to proxmox_firewall.vm_id.enable_firewall_iface_with_api.to.jsons.sh"
    exec proxmox_firewall.vm_id.enable_firewall_iface_with_api.to.jsons.sh "$@"
  else
...
printf '%s\n' "$JSON_LINE_REQ" | while IFS=$'\n' read -r CURRENT_JSON_LINE; do
  ...
  if [[ "$OUTPUT_JSON" == true ]]; then
    ...
    if [ "$_dk_rc" -ne 0 ]; then
      ...
      devkit_utils.text.echo_error.to.text.to.stderr.sh \
        "read THE CARD to tell them apart, not the guest switch : proxmox_network.vm_id.list_interfaces_vm.to.jsons.sh . On the card you named, a vm_network_firewall of 0 or absent means nothing was changed. A 1 means the flag IS set despite this error, and only the reporting failed. Then run the same command with --text to see which step stopped the play."
      devkit_utils.text.echo_error.to.text.to.stderr.sh \
