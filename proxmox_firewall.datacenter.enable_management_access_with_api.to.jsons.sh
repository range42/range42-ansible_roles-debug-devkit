...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####
# proxmox_firewall.datacenter.enable_management_access_with_api.to.jsons.sh
# Direct Proxmox HTTPS API variant of proxmox_firewall.datacenter.enable_management_access.to.jsons.sh
#
# The anti-lockout of the datacenter, the same steps as the role action
# firewall_dc_enable_management_access, field for field :
#   1. GET  ${API_URL}/cluster/firewall/rules   read before write : an accept counts as "already
...
#
# OUTPUT : one line per rule, `dc_fw_rule` says which ("api" or "ssh"). Read the `_before`
# and `_after` fields to know the state, never the `_requested` one. 99999 : no deny at all.
#
# Optional fields, as in the role : dc_fw_api_port (8006), dc_fw_ssh_port (22), dc_fw_api_pos (0),
# dc_fw_ssh_pos (1), dc_fw_mgmt_source (none : accepted from anywhere), dc_fw_api_comment,
# dc_fw_ssh_comment. The node comes from the vault ; a different proxmox_node on stdin is
...
show_example() {
  ...
  echo "    echo '{\"proxmox_node\":\"px-testing\",\"dc_fw_mgmt_source\":\"192.168.0.0/16\"}' | $(basename "$0")"
}
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Install the two rules that keep the datacenter reachable - direct Proxmox HTTPS API call ($ACTION)"
  echo
  echo OPTIONS
  echo
...
while IFS= read -r LINE ; do
  ...
  API_POS="$(printf '%s' "$REQ" | jq -r '.dc_fw_api_pos // 0 | tostring')"
  SSH_POS="$(printf '%s' "$REQ" | jq -r '.dc_fw_ssh_pos // 1 | tostring')"
  [[ "$API_POS" =~ ^[0-9]+$ ]] || API_POS=0
  [[ "$SSH_POS" =~ ^[0-9]+$ ]] || SSH_POS=1
  MGMT_SOURCE="$(printf '%s' "$REQ" | jq -r '.dc_fw_mgmt_source // empty')"
  API_COMMENT="$(printf '%s' "$REQ" | jq -r '.dc_fw_api_comment // "range42 anti-lockout, proxmox api"')"
  SSH_COMMENT="$(printf '%s' "$REQ" | jq -r '.dc_fw_ssh_comment // "range42 anti-lockout, ssh"')"
  ...
  _post_accept() {
    ...
    ')"
    _api_post "${API_URL}/cluster/firewall/rules" "$rule"
    if [[ "$HTTP_CODE" != "200" ]]; then
      _error "POST the accept on port ${dport} failed at the datacenter (http ${HTTP_CODE}) : ${BODY}"
      exit 1
    fi
  }
  ...
  for rule in api ssh ; do
    ...
    jq -nc \
      ...
      '{
        ...
        dc_fw_rule: $rule,
        dc_fw_dport: $dport,
        dc_fw_pos_requested: $pos_requested,
        dc_fw_pos_before: (if $rule == "api" then $before.api_first else $before.ssh_first end),
        dc_fw_pos_after: (if $rule == "api" then $after.api_first else $after.ssh_first end),
        dc_fw_already_present: (if $rule == "api" then $before.has_api else $before.has_ssh end),
        dc_fw_covering_deny_pos_before: (if $rule == "api" then $before.cover_api else $before.cover_ssh end),
        dc_fw_mgmt_source: (if ($src | length) > 0 then $src else null end)
      }
