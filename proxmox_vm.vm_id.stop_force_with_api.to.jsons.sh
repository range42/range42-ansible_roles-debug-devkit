...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Force-stop vm_id vm (no ACPI, server-side timeout=10) - direct Proxmox HTTPS API call ($ACTION)"
  echo
  echo OPTIONS
  echo
...
while IFS= read -r LINE ; do
  ...
  POST_RESP="$(curl -sk -X POST \
    ...
    "${BASE_URL}/qemu/${VM_ID}/status/stop" || true)"
  UPID="$(printf '%s' "$POST_RESP" | jq -r '.data // empty')"
  if [[ -z "$UPID" || "$UPID" == "null" ]]; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "POST stop did not return a UPID for vm_id ${VM_ID}. response: $POST_RESP"
    continue
  fi
