...
if [[ "${RANGE42_PROXMOX_API_FORCE:-auto}" != "off" ]]; then
  if proxmox__inc.ssh_reachable.sh ; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "hypervisor reachable over ssh - delegating to proxmox_network.sdn_subnet_cidr.delete_extra_snat_rules_with_ssh.to.jsons.sh"
    exec proxmox_network.sdn_subnet_cidr.delete_extra_snat_rules_with_ssh.to.jsons.sh "$@"
  else
