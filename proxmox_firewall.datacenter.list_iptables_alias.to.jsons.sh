...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - List the iptables aliases - datacenter firewall - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
  ...
  echo "  "
  echo "    One line per alias, each carrying dc_fw_alias_name, dc_fw_alias_cidr and"
  echo "    dc_fw_alias_comment. An empty collection prints nothing at all, which is not"
  echo "    the same as a failure."
  echo "  "
  echo EXAMPLE
  echo
  echo "$(show_example)"
