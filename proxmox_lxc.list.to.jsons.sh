...
if [ "${1:-}" = "-h" ] || [ "${1:-}" = "--help" ]; then
  ...
  echo
  echo NAME
  echo "  $(basename "$0") - list LXC status - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTION
  echo
  ...
  if [[ "$OUTPUT_JSON" == true ]]; then # json mode.
    ...
    if [[ -n "$ARG_VM_NAME_FILTER" ]]; then # check if filter provided in argument
      ...
      printf '%s\n' "$NODE_JSON" |
        ...
        jq '.[]' |
        devkit_transform.jsons.remove_key.to.jsons.sh "lxc_meta" |
        devkit_transform.jsons.key_field_greper.to.jsons.sh "lxc_name" "$ARG_VM_NAME_FILTER"

    else # not filter in argument

      # devkit_utils.text.echo_trace.to.text.to.stderr.sh "IN2 $NODE_JSON:: "
