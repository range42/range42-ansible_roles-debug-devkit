...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ] || [ $# -lt 2 ]; then
  ...
  echo
  echo NAME
  echo "  $(basename "$0") - list IPs of all VMs on a given bridge in the closest scenario manifest"
  echo
  echo OPTIONS
  echo
  ...
  echo "  then outputs one IP per line for every VM whose bridge matches <bridge>."
...
jq -r --arg b "$bridge" '.vms[] | select(.bridge == $b) | "\(.ip) # \(.vm_name)"' "$manifest"
