...
show_example() {
  ...
  local STDIN_JSON_DATA=(
    # INPUT
    '{ "vm_id":100,  "vm_fw_alias_name": "my_alias"}'
    '{ "vm_id":100, "proxmox_node": "px-testing", "vm_fw_alias_name": "my_alias" }'
...
JSON_LINE_REQ=$(devkit_proxmox.STDIN.stdin_or_jsons.to.jsons.sh "INT::vm_id" "STR::proxmox_node" "STR::vm_name" "STR::action" |
  proxmox_firewall.vm_id.list_iptables_alias.to.jsons.sh)
...
printf '%s\n' "$JSON_LINE_REQ" | while IFS=$'\n' read -r CURRENT_JSON_LINE; do
  ...
  if [[ "$OUTPUT_JSON" == true ]]; then
    ...
    printf '%s\n' "$CURRENT_JSON_LINE" |
      jq -c ' {proxmox_node, vm_id, vm_fw_alias_name }' |
