...
#
# Turn ONE NODE's firewall OFF.
#
# It does not refuse - turning a firewall off removes protection rather than access, and
# that is a legitimate operation. It READS the chain first and reports how many ACTIVE rules
# stop applying, so the size of what is being undone is visible.
#
# Note that a node is covered by TWO chains : its own and the datacenter one. Turning this
# one off does not touch the datacenter rules, which keep applying.
#
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Turn one node's firewall off - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
...
if [[ "${RANGE42_PROXMOX_API_FORCE:-auto}" != "off" ]]; then
  if proxmox__inc.api_reachable.sh ; then
    devkit_utils.text.echo_trace.to.text.to.stderr.sh "proxmox API reachable - delegating to proxmox_firewall.proxmox_node.disable_firewall_with_api.to.jsons.sh"
    exec proxmox_firewall.proxmox_node.disable_firewall_with_api.to.jsons.sh "$@"
  else
