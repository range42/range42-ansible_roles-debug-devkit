...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo "  $(basename "$0") - resume all vm - Execute the specified $ACTION action via Ansible (all vms) "
  echo
  echo SYNOPSIS
...
for VM_ID in $(
  ...
  if [[ -n "$ARG_VM_NAME_FILTER" ]]; then
    devkit_ansible.proxmox_controller.ask_vm_list_paused.to.jsons.sh "$ARG_VM_NAME_FILTER" |
      jq -r '.vm_id'
  else
    devkit_ansible.proxmox_controller.ask_vm_list_paused.to.jsons.sh |
      jq -r '.vm_id'
  fi
  ...
  if [[ "$OUTPUT_JSON" == true ]]; then
    ...
    devkit_generic.utils.text.echo_pass.to.text.to.stderr.sh "resume :: $VM_ID"
    sleep 5
  fi
