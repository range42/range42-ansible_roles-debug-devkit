...
show_example() {
  ...
  local STDIN_JSON_DATA=(
    # INPUT
    '{ "vm_id":100,  "vm_fw_alias_name": "test", "cidr": "192.168.123.0/24", "vm_fw_alias_comment": "this_comment" }'
    '{ "vm_id":100, "proxmox_node": "px-testing", "vm_fw_alias_name": "test", "vm_fw_alias_cidr": "192.168.123.0/24", }'
    '{ "vm_id":100, "proxmox_node": "px-testing", "vm_fw_alias_name": "test", "vm_fw_alias_cidr": "192.168.123.0/24", "vm_fw_alias_comment": "this_comment" }'
...
if [ "${1-}" = '-h' ] || [ "${1-}" = '--help' ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - Add iptables alias (vm) - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
