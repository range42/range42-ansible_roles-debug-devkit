...
#### #### #### #### #### #### #### #### #### #### #### #### #### #### #### ####
#
# List the SDN vnets declared at the CLUSTER level (GET /cluster/sdn/vnets).
#
...
#
set -euo pipefail
ACTION="network_list_sdn_vnets"
DEFAULT_OUTPUT_JSON=true
ARG_SDN_VNET_NAME_FILTER=""
...
show_example() {
  ...
  echo "  :: WITH A CASE INSENSITIVE FILTER ON THE VNET NAME "
  echo
  echo "    $(basename "$0") net142"
  echo "    $(basename "$0") net14 --json"
  echo
  ...
  echo "  :: PIPING "
  echo
  echo "    $(basename "$0") | jq -r '.vnet'"
  echo "    $(basename "$0") | jq -c 'select(.vnet_zone==\"r42zone\")'"
}
...
if [ "${1:-}" = "-h" ] || [ "${1:-}" = "--help" ]; then
  ...
  echo NAME
  echo
  echo "  $(basename "$0") - list the CLUSTER SDN vnets - Execute the specified $ACTION action via Ansible "
  echo
  echo OPTIONS
  echo
  echo "  $(basename "$0") [-h|--help] "
  echo "  $(basename "$0")                                        - list every SDN vnet of the cluster "
  echo "  $(basename "$0") [--json]                               - force output as json "
  echo "  $(basename "$0") [partial_or_complete_vnet] [--json]    - Force output in JSON format with a case insensitive filter on vnet "
...
while [[ $# -gt 0 ]]; do
  ...
  -*)
    ...
    ;;
  *)
    if [[ -z "$ARG_SDN_VNET_NAME_FILTER" ]]; then
      ARG_SDN_VNET_NAME_FILTER="$1"
      shift
    else
      devkit_utils.text.echo_error.to.text.to.stderr.sh "wrong number of arguments."
      show_example
...
printf '%s\n' "$JSON_LINE_REQ" | while IFS=$'\n' read -r NODE_JSON; do
  ...
  if [[ "$OUTPUT_JSON" == true ]]; then # json mode.
    ...
    if [[ -n "$ARG_SDN_VNET_NAME_FILTER" ]]; then # check if filter provided in argument

      printf '%s\n' "$NODE_JSON" |
        proxmox__inc.jsons.basic_vm_actions.to.jsons.sh "$ACTION" |
        jq -c ".[]" |
        devkit_transform.jsons.key_field_greper.to.jsons.sh "vnet" "$ARG_SDN_VNET_NAME_FILTER"
